...
function createPlayerModel(animalType, isPlayer = false) {
    ...
    try {
        ...
        body.position.y = 0.7; // Raise body off the ground
        modelGroup.add(body);
        
        // Create head
        const headGeometry = new THREE.BoxGeometry(0.8, 0.8, 0.8);
        const headMaterial = new THREE.MeshPhongMaterial({ 
            color: animals[animalType].headColor || animals[animalType].color,
            shininess: 30
        });
        ...
        const legMaterial = new THREE.MeshPhongMaterial({ 
            color: animals[animalType].legColor || animals[animalType].color,
            shininess: 30
        });
        
        // Front legs
        const frontLeftLeg = new THREE.Mesh(legGeometry, legMaterial);
        frontLeftLeg.position.set(0.4, -0.65, 0.5);
        frontLeftLeg.castShadow = true;
        frontLeftLeg.receiveShadow = true;
        body.add(frontLeftLeg);
        
        const frontRightLeg = new THREE.Mesh(legGeometry, legMaterial);
        frontRightLeg.position.set(-0.4, -0.65, 0.5);
        ...
        backLeftLeg.position.set(0.4, -0.65, -0.5);
        backLeftLeg.castShadow = true;
        backLeftLeg.receiveShadow = true;
        body.add(backLeftLeg);
        
        const backRightLeg = new THREE.Mesh(legGeometry, legMaterial);
        backRightLeg.position.set(-0.4, -0.65, -0.5);
        ...
        addAnimalFeatures(modelGroup, animalType);
        
        console.log(`${animalType} model created successfully`);
        return modelGroup;
    } catch (error) {
...
// Add animal-specific features
function addAnimalFeatures(modelGroup, animalType) {
    if (!modelGroup || !modelGroup.userData || !animalType) return;
    
    const body = modelGroup.userData.body;
...
function animateModels() {
    ...
    if (playerModel && playerModel.userData) {
        animateLegs(playerModel, playerMovement.speed);
    }
    
    // Animate simulated players
    simulatedPlayers.forEach(player => {
        if (player.model && player.model.userData) {
            animateLegs(player.model, player.walkingSpeed);
        }
    });
}
...
function animateLegs(model, speed) {
    if (!model.userData || !model.userData.legs) return;
    
    const legs = model.userData.legs;
    const walkingTime = model.userData.walkingTime || 0;
    
    // Only animate if moving
    if (Math.abs(speed) > 0.01) {
        // Update walking time
        model.userData.walkingTime = (walkingTime + speed * 5) % (Math.PI * 2);
        
        // Calculate leg angles
        const frontLeftAngle = Math.sin(walkingTime) * 0.4;
        const frontRightAngle = Math.sin(walkingTime + Math.PI) * 0.4;
        const backLeftAngle = Math.sin(walkingTime + Math.PI) * 0.4;
        const backRightAngle = Math.sin(walkingTime) * 0.4;
        
        // Apply rotations
        legs.frontLeft.rotation.x = frontLeftAngle;
        legs.frontRight.rotation.x = frontRightAngle;
        legs.backLeft.rotation.x = backLeftAngle;
        legs.backRight.rotation.x = backRightAngle;
    } else {
...
function onWindowResize() {
    if (!camera || !renderer) {
        console.warn('Cannot resize: camera or renderer not initialized');
        return;
    }
    ...
    if (gameCanvas) {
        ...
        if (rendererDom.parentElement !== gameCanvas) {
